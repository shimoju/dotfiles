...
自作版では、1行目左側と右側をレイアウトエンジンの責務として扱い、幅が足りない場合は右側の低優先度要素から省略する。幅はZshの`${(m)#...}`でsystem `wcwidth`に基づく表示カラム数を計測し、CJKや結合文字を1文字1カラムとは仮定しない。

`RPROMPT`は入力用の2行目に表示されるため使わず、`COLUMNS`から算出した空白を1行目に入れる。生の`PROMPT`は固定したレンダラー呼び出しだけを持ち、Zshがpromptを展開するたびに現在の幅で1行目を構築する。これにより、Zsh内蔵のWINCH処理による最初の再表示から新しい幅が使われ、追加の`TRAPWINCH`や二重描画を必要としない。実行済みでスクロールバックに入ったプロンプトは書き換えない。

`PROMPT_SUBST`は有効にするが、評価対象の生の`PROMPT`へpathやbranchを埋め込まない。固定した関数呼び出しがcache済みの表示データを出力し、その結果をコマンドとして再評価しない構造にする。

East Asian Ambiguous文字を常に2カラムとする端末独自設定はOSの`wcwidth`と一致しない場合がある。シェルから端末側の設定を検出する標準手段はないため、ShshはZshが返す表示幅を基準とする。
