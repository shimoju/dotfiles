...
2026-08-20から21日にかけて、パス短縮と左右セグメントの計測を文字数からsystem `wcwidth`に基づく表示カラム数へ変更した。当初は`TRAPWINCH`から再描画していたが、Zshはuser trapより先にcache済みの`PROMPT`を新しい端末幅へ描画する。縮小時には旧幅のpaddingが折り返してからShshの再描画が続くため、連続したWINCHで古いpromptが階段状に残った。

最終実装では、生の`PROMPT`を固定したレンダラー呼び出しだけにし、Zsh内蔵のprompt再展開時に現在の`COLUMNS`からpath短縮とpaddingを計算する。GitとKubernetesはcache済みデータを読むだけで、resizeによって非同期jobや外部processを追加しない。`TRAPWINCH`は不要になり、1回のWINCHに対してZsh内蔵の再描画だけが走る。

対話試験では入力bufferを置いたまま`120→110→100→90→80→70→100`列と連続変更し、各段階で旧幅のpromptを挟まず、最初の描画から新しい幅に一致することを確認した。従来のPTY試験は「最終的に正しいpromptが出力された」ことしか見ておらず、旧幅と新幅の二重描画を検出できなかったため、描画順も検査するよう改めた。

直前実装と同じLLVM cloneでClean／Dirty各30回比較した。単位はms。

| 状態 | 実装 | 初期表示 median / p95 | ブランチ表示 median / p95 | Git状態完了 median / p95 | worker内Git計算 median / p95 |
|---|---|---:|---:|---:|---:|
| Clean | 変更前 | 0.975 / 1.108 | 12.199 / 13.536 | 497.060 / 533.364 | 493.820 / 530.061 |
| Clean | 表示幅 + WINCH | 0.934 / 1.027 | 11.753 / 13.078 | 490.186 / 498.130 | 486.750 / 495.399 |
| Clean | 動的renderer | 1.998 / 2.300 | 11.857 / 12.805 | 486.954 / 493.978 | 483.845 / 490.459 |
| Dirty | 変更前 | 0.925 / 1.130 | 12.444 / 14.234 | 496.425 / 524.307 | 493.392 / 517.234 |
| Dirty | 表示幅 + WINCH | 0.932 / 1.110 | 11.896 / 13.047 | 491.772 / 497.285 | 488.562 / 494.360 |
| Dirty | 動的renderer | 2.110 / 2.225 | 12.327 / 12.946 | 491.180 / 496.697 | 488.019 / 492.142 |
